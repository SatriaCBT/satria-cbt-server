...
func NewClassController(db *gorm.DB, studentController *StudentController, teacherController *TeacherController) *ClassController {
...
func (k *ClassController) CreateClass(c *fiber.Ctx) error {
	...
	if err := k.db.First(&admin, adminID).Error; err != nil {
	...
	if err := k.db.Create(&class).Error; err != nil {
		return &fiber.Error{
			Code:    fiber.StatusInternalServerError,
			Message: err.Error(),
		}
	...
	if len(req.Teachers) > 0 {
		...
		if err := k.db.Model(&models.Teachers{}).Where("id IN ?", teacherIDs).Count(&count).Error; err != nil {
			return &fiber.Error{
				Code:    fiber.StatusInternalServerError,
				Message: err.Error(),
			}
		...
		if err := k.db.Model(&class).Association("Teachers").Append(req.Teachers); err != nil {
			return &fiber.Error{
				Code:    fiber.StatusInternalServerError,
				Message: err.Error(),
			}
	...
	if len(req.Students) > 0 {
		...
		if err := k.db.Model(&models.Students{}).Where("id IN ?", studentIDs).Count(&count).Error; err != nil {
			return &fiber.Error{
				Code:    fiber.StatusInternalServerError,
				Message: err.Error(),
			}
		...
		if err := k.db.Model(&class).Association("Students").Append(req.Students); err != nil {
			return &fiber.Error{
				Code:    fiber.StatusInternalServerError,
				Message: err.Error(),
			}
	...
	if err := k.db.Preload("Teachers.Classes").
...
func (k *ClassController) GetAllClass(c *fiber.Ctx) error {
	...
	if err := k.db.Preload("Teachers").Preload("Students").Preload("CreatedBy").Find(&classes).Error; err != nil {
		return &fiber.Error{
			Code:    fiber.StatusInternalServerError,
			Message: err.Error(),
		}
...
func (k *ClassController) GetClassByID(c *fiber.Ctx) error {
	...
	if err := k.db.Preload("Teachers").Preload("Students").Preload("CreatedBy").First(&class, classID).Error; err != nil {
		return &fiber.Error{
...
func (k *ClassController) GetClassByCode(c *fiber.Ctx) error {
	...
	var class models.Class
	if err := k.db.Preload("Teachers").Preload("Students").Preload("CreatedBy").First(&class, "code = ?", classCode).Error; err != nil {
		return &fiber.Error{
			Code:    fiber.StatusNotFound,
			Message: err.Error(),
		}
...
func (k *ClassController) UpdateClass(c *fiber.Ctx) error {
	...
	if teachers, ok := req["teachers"]; ok {
		...
		if err := k.db.Model(&models.Teachers{}).Where("id IN ?", teacherIDsParsed).Count(&count).Error; err != nil {
			return &fiber.Error{
				Code:    fiber.StatusInternalServerError,
				Message: err.Error(),
			}
		...
		if err := k.db.Model(&class).Association("Teachers").Append(teacherIDsParsed); err != nil {
			return &fiber.Error{
				Code:    fiber.StatusInternalServerError,
				Message: err.Error(),
			}
	...
	if students, ok := req["students"]; ok {
		...
		if err := k.db.Model(&models.Students{}).Where("id IN ?", studentIDsParsed).Count(&count).Error; err != nil {
			return &fiber.Error{
				Code:    fiber.StatusInternalServerError,
				Message: err.Error(),
			}
		...
		if err := k.db.Model(&class).Association("Students").Append(studentIDsParsed); err != nil {
			return &fiber.Error{
				Code:    fiber.StatusInternalServerError,
				Message: err.Error(),
			}
	...
	if err := k.db.Save(&class).Error; err != nil {
		return &fiber.Error{
			Code:    fiber.StatusInternalServerError,
			Message: err.Error(),
		}
...
func (k *ClassController) DeleteClass(c *fiber.Ctx) error {
	...
	if err := k.db.Delete(&class).Error; err != nil {
		return &fiber.Error{
			Code:    fiber.StatusInternalServerError,
			Message: err.Error(),
		}
